...
void plat_screen_flash_render( sScreenFlashDetails *p_details )
{
	...
	//	NxXbox::set_texture( 0, p_xbox_texture->GetEngineTexture()->pD3DTexture );
	...
	//NxXbox::EngineGlobals.p_Device->DrawPrimitiveUP( D3DPT_, 1, verts, sizeof( sXboxScreenFlashVert ));
}
...
#			endif // DRAW_DEBUG_LINES
			...
			for( uint32 m = 0; m < p_xbox_geom->m_num_mesh; ++m )
			{
				...
				BYTE *p_vert_data;
				p_mesh->mp_vertex_buffer[0]->Lock( 0, 0, (void**)&p_vert_data, D3DLOCK_READONLY);
				
				// Now scan through each non-degenerate tri, checking the verts to see if they are within scope.
				uint32 index0;
				uint32 index1 = p_mesh->mp_index_buffer[0]->rawdata[0];
				uint32 index2 = p_mesh->mp_index_buffer[0]->rawdata[1];
				for( uint32 i = 2; i < p_mesh->m_num_indices[0]; ++i )
				{
					// Wrap the indices round.
					index0 = index1;
					index1 = index2;
					index2 = p_mesh->mp_index_buffer[0]->rawdata[i];
