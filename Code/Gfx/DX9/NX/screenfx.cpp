...
// LWSS add
void DrawPrimitives_TriangleStrip(float a1)
{
    float v2; // [esp+4h] [ebp-78h]
    float v3; // [esp+8h] [ebp-74h]
    float v4[7]; // [esp+Ch] [ebp-70h] BYREF
    float v5[7]; // [esp+28h] [ebp-54h] BYREF
    float v6[7]; // [esp+44h] [ebp-38h] BYREF
    float v7[7]; // [esp+60h] [ebp-1Ch] BYREF

    v2 = 0.5 / (double)EngineGlobals.params.BackBufferWidth;
    v4[4] = a1;
    v3 = 0.5 / (double)EngineGlobals.params.BackBufferHeight;
    v4[0] = 0.0;
    v4[1] = 0.0;
    v4[2] = 0.0;
    v4[3] = 1.0;
    ...
    v5[0] = (float)EngineGlobals.params.BackBufferWidth;
    v5[5] = v2 + 1.0;
    qmemcpy(v6, v4, sizeof(v6));
    v6[1] = (float)EngineGlobals.params.BackBufferHeight;
    v6[6] = v3 + 1.0;
    qmemcpy(v7, v5, sizeof(v7));
    v7[1] = v6[1];
    v7[6] = v6[6];
    //return EngineGlobals_p_Device->lpVtbl->DrawPrimitiveUP(EngineGlobals_p_Device, D3DPT_TRIANGLESTRIP, 2, v4, 28);
    D3DDevice_DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, v4, 28);
}
// LWSS end
	
// LWSS Add: Some functions used by screen blur.

void BlurSetSurface2AndDepthStencil1()
{
    IDirect3DSurface9* pSurface;
    EngineGlobals.p_BlurSurface_2->GetSurfaceLevel(0, &pSurface);
    D3DDevice_SetRenderTarget(0, pSurface);
    D3DDevice_SetDepthStencilSurface(EngineGlobals.p_BlurSurface_1);
    pSurface->Release();
}
void BlurClear()
{
    BlurSetSurface2AndDepthStencil1();

    int clearColor = EngineGlobals.fog_color;
    if (!EngineGlobals.g_Registry_options_flag_1_MSAA || !EngineGlobals.g_Registry_options_flag_2)
    {
        clearColor = EngineGlobals.clear_color;
    }

    D3DDevice_Clear(0, 0, 3, clearColor, 1.0f, 0);
}

void BlurSetSurface0AndDepthStencil1()
{
    IDirect3DSurface9* pSurface;
    EngineGlobals.p_BlurSurface_0->GetSurfaceLevel(0, &pSurface);
    D3DDevice_SetRenderTarget(0, pSurface);
    D3DDevice_SetDepthStencilSurface(EngineGlobals.p_BlurSurface_1);
    pSurface->Release();
}
...
void BlurClear2()
{
    BlurSetSurface0AndDepthStencil1();
...
void RenderBlurBlank(float a1)
{
    ...
    D3DDevice_SetRenderState(D3DRS_DESTBLEND, 3);
    ...
    DrawPrimitives_TriangleStrip(a1);
}
void RenderBlurSurface2(float a1)
{
    D3DDevice_SetRenderState(D3DRS_SRCBLEND, 2);
    D3DDevice_SetRenderState(D3DRS_DESTBLEND, 2);
    D3DDevice_SetTextureStageState(0, D3DTSS_COLOROP, 4);
    D3DDevice_SetTextureStageState(0, D3DTSS_ALPHAOP, 4);
    D3DDevice_SetRenderState(D3DRS_BLENDOP, 1);
    set_texture(0, EngineGlobals.p_BlurSurface_2);
    DrawPrimitives_TriangleStrip(a1);
}
// LWSS End
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// LWSS: Done
static HRESULT focus_blur( int blurFlagsOverride )
{
    ...
    uint32 blur = (uint32)((255 - EngineGlobals.focus_blur) / 2);
    if (blur < 32)
    {
        blur = 32;
    }

    set_render_state(RS_CULLMODE, 1);
    set_render_state(RS_ZWRITEENABLE, 0);
    set_render_state(RS_ZTESTENABLE, 0);
    set_blend_mode(0);
    set_render_state(RS_UVADDRESSMODE0, 0x10001);

    DWORD alphaTest;
    DWORD srcBlend;
    DWORD destBlend;
    D3DDevice_GetRenderState(D3DRS_ALPHATESTENABLE, &alphaTest);
    ...
    if (blurFlagsOverride)
    {
        blur = 255;
    }

    RenderBlurBlank(COERCE_FLOAT((255 - blur) | (((255 - blur) | (((255 - blur) | ((255 - blur) << 8)) << 8)) << 8)));
    RenderBlurSurface2(blur | ((blur | ((blur | (blur << 8)) << 8)) << 8));

    D3DDevice_SetRenderState(D3DRS_ZENABLE, 1);
    D3DDevice_SetTextureStageState(0, D3DTSS_COLOROP, 4);
    D3DDevice_SetTextureStageState(0, D3DTSS_ALPHAOP, 4);
    D3DDevice_SetSamplerState(0, D3DSAMP_MIPFILTER, 2);
    D3DDevice_SetRenderState(D3DRS_ALPHATESTENABLE, alphaTest);
    D3DDevice_SetRenderState(D3DRS_SRCBLEND, srcBlend);
...
//    EngineGlobals.p_Device->SetRenderState( D3DRS_DESTBLEND,        D3DBLEND_INVSRCALPHA );
    ...
    return S_OK;
#endif
    // LWSS: Reworked function for PC
    D3DDevice_SetRenderTarget(0, EngineGlobals.p_RenderSurface);
    D3DDevice_SetDepthStencilSurface(EngineGlobals.p_ZStencilSurface);

    int fogColor = EngineGlobals.fog_color;

    if (!EngineGlobals.g_Registry_options_flag_1_MSAA || !EngineGlobals.g_Registry_options_flag_2)
    {
        fogColor = EngineGlobals.clear_color;
    }

    D3DDevice_Clear(0, 0, 3, fogColor, 1.0f, 0);

    set_texture(0, EngineGlobals.p_BlurSurface_0);
    set_render_state(RS_CULLMODE, 1);
    set_render_state(RS_ZWRITEENABLE, 0);
    set_render_state(RS_ZTESTENABLE, 0);

    D3DRENDERSTATETYPE renderStateRequests[5];
    renderStateRequests[0] = D3DRS_ALPHATESTENABLE;
    renderStateRequests[1] = D3DRS_SRCBLEND;
    renderStateRequests[2] = D3DRS_DESTBLEND;
    renderStateRequests[3] = D3DRS_ZENABLE;
    renderStateRequests[4] = D3DRS_ALPHABLENDENABLE;
    DWORD renderStateResults[5];
    
    for (int i = 0; i < 5; i++)
    {
        D3DDevice_GetRenderState(renderStateRequests[i], &renderStateResults[i]);
    }
    ...
    D3DDevice_SetRenderState(D3DRS_ZENABLE, 0);
    DrawPrimitives_TriangleStrip(NAN);
    ...
    for (int i = 0; i < 5; i++)
    {
        result = D3DDevice_GetRenderState(renderStateRequests[i], &renderStateResults[i]);
    }
...
void start_screen_blur( void )
{
	...
	if(( EngineGlobals.screen_blur > 0 ) && ( EngineGlobals.screen_blur_duration > 1 ))
	{
        ...
        if (EngineGlobals.use_all_blur_flags)
        {
            BlurClear();
            BlurClear2();
            BlurSetSurface2AndDepthStencil1();
...
void finish_screen_blur( void )
{
	...
	if(( EngineGlobals.screen_blur > 0 ) && ( EngineGlobals.screen_blur_duration > 1 ))
	{
        ...
        NxXbox::set_render_state(RS_ZBIAS, 0);
        NxXbox::focus_blur(EngineGlobals.use_all_blur_flags);
        NxXbox::draw_focus_effect_using_planes();
        NxXbox::set_texture(0, NULL);
        if (EngineGlobals.use_all_blur_flags)
        {
            EngineGlobals.use_all_blur_flags = 0;
        }
        // LWSS End
	}
    else if (!EngineGlobals.use_all_blur_flags)
    {
