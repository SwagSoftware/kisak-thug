...
struct sMaterial
{
	...
	sTexture*				mp_tex[MAX_PASSES]; // [0] = 0x30
	float					m_color[MAX_PASSES][4];				// Element [pass][3] holds the fixed alpha value where appropriate.
	uint32					m_reg_alpha[MAX_PASSES]; //[0]-0x80			// Low 24 bits are blend mode, high 8 bits are fixed alpha value.
	...
	D3DCOLOR /*unsigned int*/* mp_wibble_vc_colors;	// 0XF8			// Max of eight banks of vertex color wibble information.
