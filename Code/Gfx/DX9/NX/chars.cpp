...
int DwordizeTexelData(int texel_data_dword_array, int texel_data_char_array, int numBytesChar, int data)
{
	...
	for (result = 0; result < numBytesChar; ++result)
		*(_DWORD*)(texel_data_dword_array + 4 * result) = *(_DWORD*)(data
			+ 4
			* *(unsigned __int8*)(result + texel_data_char_array));
	return result;
}
// LWSS End
...
SFont* LoadFont( const char *Filename, bool memory_resident )
{
	...
	if( D3D_OK != D3DDevice_CreateTexture(Width, Height, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pFont->pD3DTexture, NULL))
	{
		Dbg_Assert( 0 );
		return NULL;
	}
	...
	if( !memory_resident )
	{
		...
		File::Read(v55, 1024, 1, p_FH); // lwss add
	}
	else
	{
	...
	do
	{
		unsigned int v30 = *(_DWORD*)v28 & 0xFF00FF00 | ((unsigned __int8)*(_DWORD*)v28 << 16) | (unsigned __int8)BYTE2(*(_DWORD*)v28);
		uint8_t v31;
		if (HIBYTE(v30) < 0x80u)
			v31 = 2 * HIBYTE(v30);
		else
			v31 = -1;
		*(_DWORD*)v28 = *(_DWORD*)v28 & 0xFF00 | (((unsigned __int8)*(_DWORD*)v28 << 16) | (unsigned __int8)BYTE2(*(_DWORD*)v28)) & 0xFFFFFF | (v31 << 24);
		v28 += 4;
		--i_;
	} while (i_);

	uint8* p_dword_texel_data = new uint8[4 * NumBytes];
	DwordizeTexelData((int)p_dword_texel_data, (int)p_temp_texel_data, NumBytes, (int)v55);
	// lwss end
	...
	memcpy(locked_rect.pBits, p_dword_texel_data, 4 * NumBytes); // Useless fucking memcpy??
	
	// lwss add
	pFont->pD3DTexture->UnlockRect(0);
	// lwss end


	// Swizzle the texture data.
	//SwizzleTexture( locked_rect.pBits, p_temp_texel_data, Width, Height, 8, Width ); // lwss remove

	// No longer need this data.
	delete[] p_temp_texel_data;	
	delete[] p_dword_texel_data; // lwss add
	
	// LWSS: Remove D3D8 palette code.
	//// Create palette.
	...
	else
	{
		...
		//Filename += ( NumChars << 3 ) + 4; // lwss remove
	}
...
/*                                                                */
/******************************************************************/
// lwss: major changes made for PC
// Done!
static uint16 sTextBuffer[0x600];
void SText::EndDraw( void )
{
	...
	{
		int v1 = 0;
		for (int i = 0; i < 0x600; i += 6)
		{
			int v3 = 4 * v1 + 2;
			sTextBuffer[i]     = 4 * v1;
			sTextBuffer[i + 1] = 4 * v1 + 1;
			sTextBuffer[i + 2] = v3;
			sTextBuffer[i + 3] = 4 * v1;
			sTextBuffer[i + 4] = v3;
			sTextBuffer[i + 5] = 4 * v1++ + 3;
		}
		hasRan = true;
	}
	...
	if( font_vertex_offset > 0 )
	{
		...
		{
			D3DDevice_DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, font_vertex_offset, font_vertex_offset / 2, sTextBuffer, D3DFMT_INDEX16, &font_vertex_buffer, 28);
		}
