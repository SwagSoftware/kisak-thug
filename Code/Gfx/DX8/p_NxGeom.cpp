...
#include <gfx/DX8/nx/instance.h>

// LWSS: xbox-only include
//#include <xbdm.h>
...
void CXboxGeom::plat_get_render_verts( Mth::Vector *p_verts )
{
	...
	if( m_mesh_array )
	{
		for( uint32 m = 0; m < m_num_mesh; ++m )
		{
			...
			p_mesh->mp_vertex_buffer[0]->Lock( 0, 0, (void**)&p_pos, D3DLOCK_READONLY);
...
void CXboxGeom::plat_get_render_colors( Image::RGBA *p_colors )
{
	...
	if( m_mesh_array )
	{
		for( uint32 m = 0; m < m_num_mesh; ++m )
		{
			...
			// LWSS: changes for DX9
			p_mesh->mp_vertex_buffer[0]->Lock( 0, 0, (void**)&p_col, D3DLOCK_READONLY );
...
void CXboxGeom::plat_set_render_verts( Mth::Vector *p_verts )
{
	...
	if( m_mesh_array )
	{
		for( uint32 m = 0; m < m_num_mesh; ++m )
		{
			...
			// LWSS: changes for DX9
			p_mesh->mp_vertex_buffer[0]->Lock( 0, 0, (void**)&p_pos, 0);
	...
	if( m_mesh_array )
	{
		for( uint32 m = 0; m < m_num_mesh; ++m )
		{
			...
			for( uint32 v = 0; v < p_mesh->m_num_vertex_buffers; ++v )
			{
				...
				// LWSS: changes for DX9
				p_mesh->mp_vertex_buffer[v]->Lock( 0, 0, (void**)&p_col, 0 );
				p_col = (Image::RGBA*)((BYTE*)p_col + p_mesh->m_diffuse_offset );
