...
IDirect3DTexture9		*pBumpTextures[17];
...
bool AddWater( Nx::CXboxGeom *p_geom, NxXbox::sMesh *p_mesh )
{
	...
	if( num_tc_sets == 1 )
	{
		...
		p_buffer0->Lock( 0, 0, (void**)&p_read_byte, D3DLOCK_READONLY);
		p_new_buffer->Lock( 0, 0, (void**)&p_write_byte, 0);
...
bool AddGrass( Nx::CXboxGeom *p_geom, NxXbox::sMesh *p_mesh )
{
	...
	for( int layer = 0; layer < grass_layers; ++layer )
	{
		...
		BYTE *p_byte_dest;
		p_grass_mesh->mp_vertex_buffer[0]->Lock( 0, 0, (void**)&p_byte_dest, 0);
