...
void sBillboardMaterialBatch::DrawIndexedVertices( int negaOne/*always -1*/, Lst::Node<sBillboardEntry>* node, sMesh* p_mesh )
{
	...
	if ( EngineGlobals.g_Registry_options_flag_1_MSAA && EngineGlobals.g_Registry_options_flag_2 )
	{
		...
		float fog_start = EngineGlobals.far_plane - (EngineGlobals.far_plane - EngineGlobals.near_plane) * 0.25f;
		float fog_end = EngineGlobals.far_plane;
		Nx::CFog::sEnableFog(true);
		D3DDevice_SetRenderState(D3DRS_FOGSTART, fog_start);
		D3DDevice_SetRenderState(D3DRS_FOGEND, fog_end);
	}
	else
	{
	...
	{
		if ( p_mesh->m_flags & sMesh::MESH_FLAG_MATERIAL_COLOR_OVERRIDE )
		{
			p_mesh->HandleColorOverride();
		}
		else
		{
			set_pixel_shader((IDirect3DPixelShader9*)p_mesh->m_pixel_shader);
		}
	}
...
void sBillboardMaterialBatch::Render( void )
{
	...
	if( mp_material )
	{
		...
		{
			// there is a 2nd Material Submit function that doesn't use shaders. (I dont care)
		}
		...
		// LWSS Add: HLSL if()
		// This isn't super-accurate, the PC port seems to do it a bit differently(wrong??)
		if (EngineGlobals.hasHLSLv101)
		{
			...
			for (int e = 0; e < 3; ++e)
			{
				if (mp_entries[e]->GetNext())
				{
					...
					set_pixel_shader((IDirect3DPixelShader9*)p_entry->mp_mesh->m_pixel_shader);
					break;
				}
			}
		}
		...
		D3DDevice_SetVertexShaderConstantF(0, (const float*)&dest_matrix, 4);
		...
		for( node = mp_entries[0]->GetNext(); node; node = next )
		{
			...
			{
				// LWSS: Logic is outsourced to a new function in PC port.
				this->DrawIndexedVertices(-1, node, p_entry->mp_mesh);
		...
		for( node = mp_entries[2]->GetNext(); node; node = next )
		{
			...
			if( p_entry->mp_mesh->m_flags & sMesh::MESH_FLAG_ACTIVE )
			{
				...
				// LWSS: Change logic here for DX9 
				vector_upload[0] = axis_right[X];
				vector_upload[1] = axis_right[Y];
				vector_upload[2] = axis_right[Z];

				vector_upload[4] = p_entry_data->m_pivot_axis[X];
				vector_upload[5] = p_entry_data->m_pivot_axis[Y];
				vector_upload[6] = p_entry_data->m_pivot_axis[Z];
