...
extern IDirect3DPixelShader9* pixel_shader_buffer_aPs11MulX2R0Rgb;
extern IDirect3DPixelShader9* pixel_shader_buffer_aPs11TexT0MulX2;
extern IDirect3DPixelShader9* pixel_shader_buffer_aPs11TexT0MulX2_0;
extern IDirect3DPixelShader9* pixel_shader_buffer_aPs11TexT0MulR0_0;

extern		IDirect3DPixelShader9* PixelShader0;
extern		IDirect3DPixelShader9* PixelShader0IVA;
extern		IDirect3DPixelShader9* PixelShader1;
extern		IDirect3DPixelShader9* PixelShader2;
extern		IDirect3DPixelShader9* PixelShader3;
extern		IDirect3DPixelShader9* PixelShader4;
extern		IDirect3DPixelShader9* PixelShader5;
extern		IDirect3DPixelShader9* PixelShaderBrighten;
extern		IDirect3DPixelShader9* PixelShaderBrightenIVA;
extern		IDirect3DPixelShader9* PixelShaderFocusBlur;
extern		IDirect3DPixelShader9* PixelShaderFocusIntegrate;
extern		IDirect3DPixelShader9* PixelShaderFocusLookupIntegrate;
extern		IDirect3DPixelShader9* PixelShaderNULL;
extern		IDirect3DPixelShader9* PixelShaderPointSprite;
extern		IDirect3DPixelShader9* PixelShaderBumpWater;
...
namespace NxXbox
{
	struct sTextureProjectionDetails
	{
		...
		sScene			*p_scene;
		XGMATRIX		view_matrix;	   // +12
		XGMATRIX		projection_matrix; // +76
		XGMATRIX		texture_projection_matrix; // +140
	};
	...
	// lwss: shitty helper function
	//void		set_pixel_shader(uint32 shader_id)
	//{
	//	set_pixel_shader((IDirect3DPixelShader9*)shader_id);
	//}
	...
	//void		set_texture( uint32 pass, IDirect3DTexture8 *p_texture ); // LWSS add (no palette on PC)
	void		set_texture( uint32 pass, IDirect3DTexture9 *p_texture ); // LWSS add (no palette on PC)
	...
	bool	check_radius(D3DXVECTOR3* p_center, float radius); // LWSS add
	float	get_bounding_sphere_nearest_z( void );
	bool	IsVisible( Mth::Vector &center, float radius );
	void	render_shadow_targets();
	//void	render_light_glows( bool test ); // LWSS: Seems removed in PC
