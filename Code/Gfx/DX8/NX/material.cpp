...
void sMaterial::Submit( void )
{
	...
	else if (EngineGlobals.specular_enabled > 0)
	{
	...
	for( p = 0; p < m_passes; ++p )
	{
		if( !( EngineGlobals.texture_stage_override & ( 1 << p )))
		{
			...
			if( mp_tex[p] )
			{
				...
				set_texture( p, mp_tex[p]->pD3DTexture ); // LWSS: no palette on PC
...
//				set_render_state( RS_MINMAGFILTER0 + p,		m_filtering_mode[p] );
				...
				if( m_flags[p] & MATFLAG_BUMP_SIGNED_TEXTURE )
				{
					...
					if( EngineGlobals.color_sign[p] != 0xE0000000 )// LWSS: changed flag here... unsure what it is exactly ( D3DTSIGN_RSIGNED | D3DTSIGN_GSIGNED | D3DTSIGN_BSIGNED ))
					{
						EngineGlobals.color_sign[p] = 0xE0000000; // LWSS: Changed flag (D3DTSIGN_RSIGNED | D3DTSIGN_GSIGNED | D3DTSIGN_BSIGNED);
						//D3DDevice_SetTextureStageState( p, D3DTSS_COLORSIGN, D3DTSIGN_RSIGNED | D3DTSIGN_GSIGNED | D3DTSIGN_BSIGNED );  // LWSS: Removed
					}
				}
				else if (EngineGlobals.color_sign[p]) // LWSS: changed if()
				{
					//if( EngineGlobals.color_sign[p] != ( D3DTSIGN_RUNSIGNED | D3DTSIGN_GUNSIGNED | D3DTSIGN_BUNSIGNED ))
					//{
						EngineGlobals.color_sign[p] = 0;  // (D3DTSIGN_RUNSIGNED | D3DTSIGN_GUNSIGNED | D3DTSIGN_BUNSIGNED); // LWSS: Changed to Zero
						//D3DDevice_SetTextureStageState( p, D3DTSS_COLORSIGN, D3DTSIGN_RUNSIGNED | D3DTSIGN_GUNSIGNED | D3DTSIGN_BUNSIGNED ); // LWSS: Removed
					//}
				}
				...
				if(( m_flags[p] & MATFLAG_ENVIRONMENT ) && ( EngineGlobals.allow_envmapping ))
				{
					...
					// LWSS: add with PC port
					set_render_state(RS_LOCALVIEWER, 1);
				...
				else if( m_flags[p] & MATFLAG_UV_WIBBLE )
				{
					...
					//else
					//{
						// LWSS: Add more here for PC
						uv_mat._11 = mp_UVWibbleParams[p]->m_UVMatrix[0];
						uv_mat._12 = mp_UVWibbleParams[p]->m_UVMatrix[1];
						uv_mat._22 = mp_UVWibbleParams[p]->m_UVMatrix[0];
						uv_mat._21 = -mp_UVWibbleParams[p]->m_UVMatrix[1];
						// LWSS end
				...
				else
				{
					...
					DWORD tex_coord_index, tex_trans_flags;
			...
			else
			{
				...
				set_texture( p, NULL );  //LWSS: No palette on PC
			}
		}
	}
