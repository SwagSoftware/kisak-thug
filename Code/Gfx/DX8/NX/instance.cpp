...
Mth::Matrix* pBoneTransforms			= NULL;
...
void render_instances( uint32 flags )
{
	...
	if( current_index > 0 )
	{
		...
		// lwss: Moved these D3D calls above the for-loop below.
...
/*                                                                */
/******************************************************************/
// lwss: changed this function a lot
// lwss: I think this is done.
void CInstance::Render( uint32 flags )
{
	...
	{
		bool isLightingAllowed = ((m_flags & CInstance::INSTANCE_FLAG_LIGHTING_ALLOWED) == 0);
		// Is a non-skinned object.
		pLastBoneTransforms = NULL;
		
		// Do the lighting setup here.
		if( isLightingAllowed || ( GetScene()->m_flags & SCENE_FLAG_RENDERING_SHADOW ))
		{
			D3DDevice_SetRenderState( D3DRS_LIGHTING, FALSE );
		}
		else
	...
	else
	{
		...
		D3DDevice_SetTransform(D3DTS_WORLDMATRIX(0), (const D3DMATRIX*)&m_transform);
		pBoneTransforms = mp_bone_transforms;
		numBones = m_num_bones;
		renderFlags = m_flags;
...
void CInstance::RenderShadowVolume( void )
{
	if( GetBoneTransforms())
	{
		...
		for( int m = 0; m < p_scene->m_num_mesh_entries; ++m )
		{
			...
			BYTE* p_byte;
			p_mesh->mp_vertex_buffer[0]->Lock( 0, 0, (void**)&p_byte, D3DLOCK_READONLY);
