...
#include	"gfx\DX8\p_NxSector.h"
#include	"gfx\DX8\p_NxScene.h"
#include	"gfx\DX8\p_NxModel.h"
#include	"gfx\DX8\p_NxGeom.h"
#include	"gfx\DX8\p_NxMesh.h"
#include	"gfx\DX8\p_NxSprite.h"
#include	"gfx\DX8\p_NxTexture.h"
#include	"gfx\DX8\p_NxParticle.h"
#include	"gfx\DX8\p_NxTextured3dPoly.h"
#include	"gfx\DX8\p_NxNewParticleMgr.h"
#include	"gfx\DX8\p_NxWeather.h"
#include	"core\math.h"
...
static void plat_post_render_DrawPrimitiveUP()
{
	...
	v6 = (float)NxXbox::EngineGlobals.backbuffer_height;
	v2 = v6;
	v7 = 0.25 * v6 * 0.5;
	v8[0] = 0.0;
	v8[1] = 0.0;
...
void CEngine::s_plat_post_render( void )
{
	...
	if( !NxXbox::EngineGlobals.loadingscreen_visible )
	{
		...
		{
			plat_post_render_DrawPrimitiveUP();
		}

		D3DDevice_EndScene();
		//D3DDevice_Swap( D3DSWAP_DEFAULT );

		if (!NxXbox::EngineGlobals.focus_blur || NxXbox::EngineGlobals.focus_blur_duration)
		{
		...
		if (NxXbox::EngineGlobals.focus_blur || NxXbox::EngineGlobals.focus_blur_duration < 10)
		{
			if (NxXbox::EngineGlobals.letterbox_active)
			{
				if (NxXbox::EngineGlobals.g_Registry_options_flag_1_MSAA && NxXbox::EngineGlobals.g_Registry_options_flag_2)
				{
					D3DDevice_Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, NxXbox::EngineGlobals.fog_color, 1.0f, 0);
					return;
				}

				D3DDevice_Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, NxXbox::EngineGlobals.clear_color, 1.0f, 0);
				return;
			}
			if (NxXbox::EngineGlobals.clear_color_buffer)
			{
				if (NxXbox::EngineGlobals.g_Registry_options_flag_1_MSAA && NxXbox::EngineGlobals.g_Registry_options_flag_2)
				{
					D3DDevice_Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, NxXbox::EngineGlobals.fog_color, 1.0f, 0);
			...
			D3DDevice_Clear(0, 0, D3DCLEAR_ZBUFFER, NxXbox::EngineGlobals.clear_color, 1.0f, 0);
		}
		...
		for( int v = 0; v < num_viewports; ++v )
		{
			...
			for( int i = 0; i < MAX_LOADED_SCENES; i++ )
			{
				if( sp_loaded_scenes[i] )
				{
					...
					if( !pXboxScene->IsSky())
					{
						...
						NxXbox::sScene* p_xbox_scene = pXboxScene->GetEngineScene();
						if ((p_xbox_scene->m_flags & SCENE_FLAG_RECEIVE_SHADOWS) == 0)
						{
			...
			for( int i = 0; i < MAX_LOADED_SCENES; i++ )
			{
				if( sp_loaded_scenes[i] )
				{
					...
					if( pXboxScene->IsSky())
					{
						// LWSS: Add registry config if() here
						if (!NxXbox::EngineGlobals.g_Registry_options_flag_2 || !NxXbox::EngineGlobals.g_Registry_options_flag_1_MSAA)
						{
							DWORD stage_zero_minfilter = 0;
							DWORD stage_zero_mipfilter = 0;
							// LWSS: Convert to DX9 Samplers
							D3DDevice_GetSamplerState(0, D3DSAMP_MINFILTER, &stage_zero_minfilter);
							D3DDevice_GetSamplerState(0, D3DSAMP_MIPFILTER, &stage_zero_minfilter);
							D3DDevice_SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
							D3DDevice_SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
							...
							D3DDevice_SetSamplerState(0, D3DSAMP_MIPFILTER, stage_zero_mipfilter);
...
const char* CEngine::s_plat_get_platform_extension( void )
{
	...
	// LWSS: Confirmed to still be Xbx on PC
	// LWSS2: WEIRD none of the datafiles have the extension and it fails to open them due to the path.
	return "Xbx";
}
...
void CEngine::s_plat_set_screen_blur( uint32 amount )
{
	...
	// LWSS: Change to blurSurface2
	if( NxXbox::EngineGlobals.p_BlurSurface_2 )
	{
		//NxXbox::EngineGlobals.screen_blur = amount;
		// LWSS: Change to focus blur
		NxXbox::EngineGlobals.focus_blur = amount;
	}
} 
