...
class CXBSound
{
	...
	WAVEFORMATEX		m_WaveFormat;				// This encapsulates WAVEFORMATEX.
    DSBUFFERDESC			m_dsbd;
    ...
    bool	Create( const WAVEFORMATEX* pwfxFormat, DWORD dwFlags, VOID* pBuffer, DWORD dwBytes );
    void    Destroy();
...
bool CXBSound::Create( const CHAR* strFileName, DWORD dwFlags )
{
    ...
    br = waveFile.GetFormat( &m_WaveFormat, sizeof( WAVEFORMATEX ));
    if( !br )
	{
        return false;
	}
...
bool CXBSound::Create( const WAVEFORMATEX* pwfxFormat, DWORD dwFlags, VOID* pBuffer, DWORD dwBytes )
{
	...
	//if( pwfxFormat.wFormatTag == WAVE_FORMAT_XBOX_ADPCM )
	//{
