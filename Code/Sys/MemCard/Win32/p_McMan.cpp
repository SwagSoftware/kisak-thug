...
{
	// LWSS: modify for PC
	ULARGE_INTEGER FreeBytesAvailableToCaller; // [esp+0h] [ebp-14h] BYREF
	ULARGE_INTEGER TotalNumberOfBytes; // [esp+8h] [ebp-Ch] BYREF

	if( m_mounted_drive_letter && GetDiskFreeSpaceExA(0, &FreeBytesAvailableToCaller, &TotalNumberOfBytes, 0))
	{
		return (FreeBytesAvailableToCaller.LowPart >> 10) + (FreeBytesAvailableToCaller.HighPart << 22);
		//char p_drive[20];
		//strcpy(p_drive,"z:\\");
		//
...
{
	// lwss: New code for PC
	_WIN32_FIND_DATAA data;
	sprintf(cardFilenameBuffer, "Save\\%s", mask);
	HANDLE find = FindFirstFileA(cardFilenameBuffer, &data);
	...
	if (find != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			strncpy(new_file->m_Filename, data.cFileName, 31);
			strncpy(new_file->m_DisplayFilename, data.cFileName, 62);
			...
			new_file->m_Size = data.nFileSizeLow;
