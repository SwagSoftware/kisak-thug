...
void Device::read_data ( void )
{
	...
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_START) ? (1 << 3) : 0;		// XBox 'Start' = PS2 'Start'.
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_BACK) ? (1 << 0) : 0;		// XBox 'Back' = PS2 'Select'.
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_LEFTSTICK) ? (1 << 1) : 0;	// Xbox 'Left Stick Button' = PS2 'Left Stick Button'.
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_RIGHTSTICK) ? (1 << 2) : 0;	// Xbox 'Right Stick Button' = PS2 'Right Stick Button'.
	...
	m_data.m_control_data[3] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_Y) ? (1 << 4) : 0;	// XBox 'Y' = PS2 'Triangle'.
	m_data.m_control_data[3] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_B) ? (1 << 5) : 0;	// XBox 'B' = PS2 'Circle'.
	m_data.m_control_data[3] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_A) ? (1 << 6) : 0;	// XBox 'A' = PS2 'X'.
	m_data.m_control_data[3] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_X) ? (1 << 7) : 0;	// XBox 'X' = PS2 'Square'.
...
//		m_data.m_control_data[3] ^= ( xis.Gamepad.bAnalogButtons[XINPUT_GAMEPAD_BLACK] > 0x20 ) ? (( 1 << 2 ) | ( 1 << 3 )) : 0;	// XBox 'Black'= PS2 'L1' + PS2 'R1'.
		...
		// (SDL) Triggers range from 0 when released to 32767 when fully pressed
		m_data.m_control_data[3] ^= (SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_TRIGGERLEFT) > 16384) ? (1 << 0) : 0;  // XBox 'Left Trigger pressed down > halfway' = PS2 'L2'.
		m_data.m_control_data[3] ^= (SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_TRIGGERRIGHT) > 16384) ? (1 << 1) : 0; // XBox 'Right Trigger pressed down > halfway' = PS2 'R2'.

		m_data.m_control_data[3] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_LEFTSHOULDER) ? (1 << 2) : 0;				// XBox 'Left Trigger' = PS2 'L1'.
		m_data.m_control_data[3] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_RIGHTSHOULDER) ? (1 << 3) : 0;				// XBox 'Right Trigger' = PS2 'R1'.
	...
	// lwss: 0 out analog buttons.
	m_data.m_control_data[12] = 0;
	m_data.m_control_data[13] = 0;
	m_data.m_control_data[14] = 0;
	m_data.m_control_data[15] = 0;
	m_data.m_control_data[16] = 0;
	m_data.m_control_data[17] = 0;
	...
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_UP) ? (1 << 4) : 0;		// XBox 'DPad Up' = PS2 'DPad Up'.
	m_data.m_control_data[10] = SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_UP) ? 0xFF : 0;			// XBox 'DPad Up' = PS2 'DPad Up'.
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_RIGHT) ? (1 << 5) : 0;	// XBox 'DPad Right' = PS2 'DPad Right'.
	m_data.m_control_data[8] = SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_RIGHT) ? 0xFF : 0;			// XBox 'DPad Right' = PS2 'DPad Right'.
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_DOWN) ? (1 << 6) : 0;	// XBox 'DPad Down' = PS2 'DPad Down'.
	m_data.m_control_data[11] = SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_DOWN) ? 0xFF : 0;			// XBox 'DPad Down' = PS2 'DPad Down'.
	m_data.m_control_data[2] ^= SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_LEFT) ? (1 << 7) : 0;	// XBox 'DPad Left' = PS2 'DPad Left'.
	m_data.m_control_data[9] = SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_LEFT) ? 0xFF : 0;			// XBox 'DPad Left' = PS2 'DPad Left'.
	...
	//  (SDL) For thumbsticks, the state is a value ranging from -32768 (up/left) to 32767 (down / right). (Seems to be the same as xbox)
	SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_RIGHTX);

	m_data.m_control_data[4] = XboxThumbToPS2Thumb(SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_RIGHTX));	// Analog stick right (X direction).
	m_data.m_control_data[5] = XboxThumbToPS2Thumb(-SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_RIGHTY));	// Analog stick right (Y direction).
	m_data.m_control_data[6] = XboxThumbToPS2Thumb(SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_LEFTX));	// Analog stick left (X direction).
	m_data.m_control_data[7] = XboxThumbToPS2Thumb(SDL_GameControllerGetAxis((SDL_GameController*)m_data.m_handle, SDL_CONTROLLER_AXIS_LEFTY));	// Analog stick left (Y direction).
	...
	if (SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_START)
		|| SDL_GameControllerGetButton((SDL_GameController*)m_data.m_handle, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_A))
	{
...
void Device::Acquire( void )
{
	if (m_state == vIDLE)
	{
		// LWSS: Called like this:
		// first call: port=0 slot=0
		// second call: port=1 slot=0
		Dbg_Message( "Acquiring controller port %d slot %d\n", m_data.m_port, m_data.m_slot );

		if (SDL_NumJoysticks() < m_data.m_port + 1)
		{
			return;
		}

		if (!SDL_IsGameController(m_data.m_port))
		{
			return;
		}

		m_data.m_handle = SDL_GameControllerOpen(m_data.m_port);
		memset(m_data.m_control_data, 0x00, CTRL_BUFFER_LENGTH);
		m_data.m_valid = false;
		//m_state = vACQUIRING;
		m_state = vACQUIRED;
...
void Device::Unacquire ( void )
{
    ...
    {
		SDL_GameControllerClose((SDL_GameController*)m_data.m_handle);
		m_data.m_handle = NULL;
    }
